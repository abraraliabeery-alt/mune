...
@section('content')
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-semibold">{{ __('messages.admin_order_title', ['code' => (string) $order->public_code]) }}</h1>
            <p class="text-sm opacity-75">{{ __('messages.admin_order_subtitle', ['id' => (int) $order->id]) }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-2 rounded-2xl p-4 sm:p-5 {{ $theme === 'dark' ? 'glass' : 'glass-light' }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <div class="text-xs opacity-70">{{ __('messages.orders_status') }}</div>
                    <div class="mt-1 font-semibold">{{ __('messages.orders_status_' . (string) $order->status) }}</div>
                </div>
                <div>
            ...
            @if ((string) $order->delivery_method === 'delivery')
                <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <div class="text-xs opacity-70">{{ __('messages.orders_delivery_name') }}</div>
                        <div class="mt-1 font-semibold">{{ (string) $order->customer_name }}</div>
                    </div>
                    <div>
                        <div class="text-xs opacity-70">{{ __('messages.orders_delivery_phone') }}</div>
                        <div class="mt-1 font-semibold">{{ (string) $order->phone }}</div>
                    </div>
                </div>
                <div class="mt-4">
                    <div class="text-xs opacity-70">{{ __('messages.orders_delivery_address') }}</div>
            ...
            @else
                <div class="mt-4">
                    <div class="text-xs opacity-70">{{ __('messages.orders_table') }}</div>
                    <div class="mt-1 font-semibold">{{ $order->table_number ?? '-' }}</div>
                </div>
            @endif

            @if (!empty($order->notes))
                <div class="mt-4">
                    <div class="text-xs opacity-70">{{ __('messages.orders_notes') }}</div>
        ...
        <aside class="lg:col-span-1">
            <div class="rounded-2xl p-4 {{ $theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-white/75 border border-slate-900/10' }}">
                ...
                <div class="mt-4">
                    <form method="POST" action="{{ route('admin.orders.status', ['order' => $order->id]) }}">
                        @csrf
                        @method('PATCH')

                        <label class="text-xs opacity-70" for="status">{{ __('messages.admin_order_update_status') }}</label>
                        <select id="status" name="status" class="mt-2 w-full px-3 py-2 rounded-xl text-sm {{ $theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-white/80 border border-slate-900/10' }}">
                            @foreach (['new','preparing','ready','delivered','cancelled'] as $st)
                                <option value="{{ $st }}" @selected($order->status === $st)>{{ __('messages.orders_status_' . $st) }}</option>
                            @endforeach
                        </select>

                        <button type="submit" class="mt-3 w-full px-4 py-3 rounded-xl text-sm {{ $theme === 'dark' ? 'chip hover:bg-white/10' : 'chip-light hover:bg-white/70' }}">
